...
import AddTrainerDialog from './components/add';
import ViewTrainerDialog from './components/view';
import EditTrainerDialog from './components/edit';
import DeleteTrainerDialog from './components/delete';
...
type PageProps = {
  trainers: Paginator<TrainerRow>;
};

const breadcrumbs: BreadcrumbItem[] = [{ title: 'Trainers', href: trainers.index().url }];

export default function TrainersIndex() {
  const { props } = usePage<PageProps>();
  const pager = props.trainers;
  ...
  const navigate = (page: number | null) => {
    ...
    const url = `${trainers.index().url}${params.toString() ? `?${params}` : ''}`;
              ...
              {pager.data.map((t) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell className="font-medium">
                    <ViewTrainerDialog trainer={t} trigger={<span className="hover:underline cursor-pointer">{t.name}</span>} />
                  </TableCell>
                  ...
                  <TableCell className="text-right space-x-2">
                    <EditTrainerDialog trainer={t} />
                    <DeleteTrainerDialog trainer={t} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
